...
class ExhibitAdmin(admin.ModelAdmin):
    ...
    def get_urls(self):  # type: ignore
        return [
            path(
                "<id>/password/",
                self.admin_site.admin_view(self.exhibit_change_password),
            ),
        ] + super().get_urls()
    ...
    def exhibit_change_password(
        ...
        if request.method == "POST":
            form = self.change_password_form(exhibit, request.POST)
            if form.is_valid():
                form.save()
                change_message = self.construct_change_message(request, form, None)
                self.log_change(request, exhibit, change_message)
                ...
                return HttpResponseRedirect(
                    reverse(
                        ...
                        % (
                            ...
                            exhibit._meta.app_label,
                            exhibit._meta.model_name,
                        ),
                        args=(exhibit.pk,),
                    )
                )
        else:
            form = self.change_password_form(exhibit)

        fieldsets = [(None, {"fields": list(form.base_fields)})]
        admin_form = admin.helpers.AdminForm(form, fieldsets, {})

        context = {
            "title": _("Change password: %s") % escape(exhibit.get_username()),
